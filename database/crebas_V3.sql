...
drop table if exists porforasi;
...
(
   id_numerator       int not null auto_increment,
   nama_numerator     varchar(255),
   primary key (id_numerator)
);
...
(
   id_porforasi       int not null auto_increment,
   nama_porforasi     varchar(255),
   primary key (id_porforasi)
);
...
create table plong
(
   id_plong       int not null auto_increment,
   nama_plong     varchar(255),
   primary key (id_plong)
);
...
alter table proses add constraint fk_proses_numerator foreign key (id_numerator)
      references numerator (id_numerator) on delete restrict on update restrict;

alter table proses add constraint fk_proses_porforasi foreign key (id_porforasi)
      references porforasi (id_porforasi) on delete restrict on update restrict;

alter table proses add constraint fk_proses_plong foreign key (id_plong)
      references plong (id_plong) on delete restrict on update restrict;
...
(2, 2, 0, 0, 'Tinta Hitam', 30, 50, 40, 'Hitam', 120000, 130000, 10),
(3, 1, 1, 2, 'Kertas A4', 57, 150, 152, 'Kuning', 1000, 1500, 100);

INSERT INTO `history` (`id_history`, `id_varian`, `tanggal`, `deskripsi`) VALUES
(1, 3, '2018-08-20 16:10:17', '+ 150 (awal)'),
(3, 3, '2018-08-20 16:17:06', '+ 12 (update)');